...
Transpose moves the notes. This is the global version, so it retunes the notes on
...
The scale is set on the screen, which responds to gestures, so the controls follow
whichever input map is active.
...
```bash
cd samples/user-scale-global
cmake -B build -DCMAKE_TOOLCHAIN_FILE=../../cmake/operator-mode-toolchain.cmake
cmake --build build
```

This produces `build/user_scale.opm`. Copy it into `/global/` on the Operator's
USB drive, then reboot to load it.
