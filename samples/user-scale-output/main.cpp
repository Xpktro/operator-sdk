// User Scale (Output) sample mode.
//
// A shell around the body both User Scale modes share. The params are declared
// with the per-instance bit set, so each output running the mode keeps a scale of
// its own, and the calls below hand each entry point straight over.

#include "../user-scale-common/user_scale_mode.h"

OP_USER_SCALE_DEFINE_PARAMS(1);
